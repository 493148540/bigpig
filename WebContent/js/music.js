...
function AutoGo(){ 
	...
	if(index==spanList.length){
		spanList[0].className="selected"; //回到初始
	}else{ 
		spanList[index].className="selected";//选择第一张
	} 
	...
	timer=setInterval(function(){
		...
		if (t>maxT) {//当图片到达终点停止计时器
			...
			inner.style.left=change/maxT*t+start+"px";//每隔17毫秒让块移动,定位
			if (index==spanList.length&&t>=maxT) {
				inner.style.left=0;
				index=0;//当图片到最后一张时把它瞬间切换到第一张，由于都通一张图片不会影响效果
			}
	},17);
	} 
	//编写图片向右滑动的函数
