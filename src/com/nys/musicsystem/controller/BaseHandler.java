...
import com.nys.musicsystem.querydomain.QueryPage;
import com.nys.musicsystem.service.BaseService;
...
public class BaseHandler<T,E extends QueryPage> {
	...
	public Info edit(T t,
		...
		try{
			return new Info(service.updateExcludeNull(t),"编辑成功",true);
		}catch(Exception e){
			return new Info(t,"编辑失败,系统抛出了异常:"+e,false);
		}
	...
	public Info add(T t,
			...
			try{
				return new Info(service.insert(t),"编辑成功",true);
			}catch(Exception e){
				return new Info(t,"编辑失败,系统抛出了异常:"+e,false);
			}
	...
	public Info remove(@RequestParam(required = true,value="id")Long id){
		...
		try{
			if(service.deleteById(id)>0)
				return new Info(service.deleteById(id),"删除成功",true);	
			else
				return new Info(false,"删除成功",true);	
		}catch(Exception e){
			return new Info(false,"编辑失败,系统抛出了异常:"+e,false);
		}
	...
	public Info findById(Long id){
		...
		return new Info(service.findById(id),"查询成功",true);
	} 
}
